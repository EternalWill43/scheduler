...
export const addEmployee = async (firstName, lastName, departmentName, shiftName, day1Name, day2Name) => {
...
export const removeEmployee = async (firstName, lastName) => {
    let [rows] = await connection
        .promise()
        .query("DELETE FROM employees where first_name like ? and last_name like ?;", [firstName, lastName]);
    return rows;
};

export const editEmployee = async (firstName, lastName, departmentName, shiftName, day1Name, day2Name) => {
    let [rows] = await connection
        .promise()
        .query(
